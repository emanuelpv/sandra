...
use App\Models\AgendamentosExamesListaMembroModel;

use App\Models\AgendamentosExamesListaModel;

class AgendamentosExamesLista extends BaseController
{

	protected $AgendamentosExamesListaModel;
	protected $AgendamentosExamesListaMembroModel;
	...
	public function __construct()
	{
		...
		$this->AgendamentosExamesListaMembroModel = new AgendamentosExamesListaMembroModel();
	...
	public function index()
	{
		...
		$permissao = verificaPermissao('AgendamentosExamesLista', 'listar');
		if ($permissao == 0) {
			echo mensagemAcessoNegado(session()->organizacoes);
			$this->LogsModel->inserirLog('Acesso indevido ao Módulo AgendamentosExamesLista', session()->codPessoa);
			exit();
		}
		...
		$data = [
			'controller'    	=> 'agendamentosExamesLista',
			'title'     		=> 'ExamesLista'
		];
		echo view('tema/cabecalho');
		echo view('tema/menu_vertical');
		echo view('tema/menu_horizontal');
		return view('agendamentosExamesLista', $data);
	}
	...
	public function getAll()
	{
		...
		$result = $this->AgendamentosExamesListaModel->pegaAgendamentosExamesLista();
		...
		foreach ($result as $key => $value) {
			...
			if ($this->AgendamentosExamesListaModel->pegaConselho($value->codConselho) == NULL) {
				$codConselho = null;
			} else {
				$codConselho = $this->AgendamentosExamesListaModel->pegaConselho($value->codConselho)->nomeConselho;
			}
	...
	public function pegaMembro()
	{
		...
		if ($this->validation->check($id, 'required|numeric')) {
			...
			$data = $this->AgendamentosExamesListaMembroModel->pegaPorCodigo($id);
	...
	public function pegaMembros($codExameLista = null)
	{
		...
		$result = $this->AgendamentosExamesListaModel->pegaMembros($codExameLista);
		...
		foreach ($result as $key => $value) {
			...
			$ops = '<div class="btn-group">';
			$ops .= '	<button type="button" class="btn btn-xs btn-primary" onclick="editagendamentosExamesListaMembro(' . $value->codExameListaMembro . ')"><i class="fa fa-edit"></i></button>';
			$ops .= '	<button type="button" class="btn btn-xs btn-danger"  data-toggle="tooltip" data-placement="top" title="Remover" onclick="removeagendamentosExamesListaMembro(' . $value->codExameListaMembro . ')"><i class="fa fa-trash"></i></button>';
			$ops .= '</div>';
	...
	public function listaDropDownEstadosFederacao()
	{
		...
		$result = $this->AgendamentosExamesListaModel->listaDropDownEstadosFederacao();
	...
	public function importarAgendamentosExamesLista()
	{
		...
		$agendamentosExamesListaApolo = $this->AgendamentosExamesListaModel->agendamentosExamesListaApolo();


		foreach ($agendamentosExamesListaApolo as $especilidadeApolo) {
			$fields = array();
			...
			$especilidade = $this->AgendamentosExamesListaModel->pegaexameListaPorNome($especilidadeApolo->nome_esp);

			if ($especilidade !== NULL and $especilidade->codExameLista !== NULL) {
				$this->AgendamentosExamesListaModel->update($especilidade->codExameLista, $fields);
			} else {
				$this->AgendamentosExamesListaModel->insert($fields);
			}
		}
		...
		//IMPORTAR MEMBROS
		$membrosAgendamentosExamesListaApolo = $this->AgendamentosExamesListaModel->membrosAgendamentosExamesListaApolo();


		foreach ($membrosAgendamentosExamesListaApolo as $membroEspecilidadeApolo) {
			...
			if ($membros['codPessoa'] !== NULL and $membros['codExameLista'] !== NULL) {
				...
				$membrosEspecilidade = $this->AgendamentosExamesListaModel->pegaMembroPorExameListaECodPessoa($membros['codExameLista'], $membros['codPessoa']);

				if ($membrosEspecilidade !== NULL) {
					@$this->AgendamentosExamesListaMembroModel->update($membrosEspecilidade->codExameListaMembro, $membros);
				} else {

					@$this->AgendamentosExamesListaMembroModel->insert($membros);
				}
			}
		}
		...
		$response['messages'] = 'Importação de agendamentosExamesLista e membros realizada com sucesso';
	...
	public function getOne()
	{
		...
		if ($this->validation->check($id, 'required|numeric')) {
			...
			$data = $this->AgendamentosExamesListaModel->where('codExameLista', $id)->first();
	...
	public function add()
	{
		...
		if ($this->validation->run($fields) == FALSE) {
			...
			if ($this->AgendamentosExamesListaModel->insert($fields)) {
	...
	public function editMembro()
	{
		...
		if ($this->validation->run($fields) == FALSE) {
			...
			if ($this->AgendamentosExamesListaMembroModel->update($fields['codExameListaMembro'], $fields)) {
	...
	public function AddMembro()
	{
		...
		$existe = $this->AgendamentosExamesListaMembroModel->pegaExameListaPorMembro($fields['codExameLista'], $fields['codPessoa']);
		...
		if ($this->validation->run($fields) == FALSE) {
			...
			if ($this->AgendamentosExamesListaMembroModel->insert($fields)) {
	...
	public function listaDropDownAgendamentosExamesLista()
	{

		$result = $this->AgendamentosExamesListaModel->listaDropDownAgendamentosExamesLista();
	...
	public function listaDropDownAgendamentosExamesListaDisponivelMarcacao()
	{
		...
		$result = $this->AgendamentosExamesListaModel->listaDropDownAgendamentosExamesListaDisponivelMarcacao();
	...
	public function statusMinhasAgendamentosExamesLista()
	{
		$response = array();

		if (empty(session()->minhasAgendamentosExamesLista)) {
			$statusMinhasAgendamentosExamesLista = 0;
		} else {

			$statusMinhasAgendamentosExamesLista = 1;
			...
			foreach (session()->minhasAgendamentosExamesLista as $minhaExameLista) {
		...
		$response['success'] = true;
		$response['csrf_hash'] = csrf_hash();
		$response['statusMinhasAgendamentosExamesLista'] = $statusMinhasAgendamentosExamesLista;
	...
	public function listaDropDownEspecialistasDisponivelMarcacaoManual()
	{
		...
		$response = array();
		$codExameLista = $this->request->getPost('codExameLista');
		$especialistas = $this->AgendamentosExamesListaModel->listaDropDownEspecialistasDisponivelMarcacao($codExameLista);
	...
	public function listaDropDownConselhos()
	{
		...
		$result = $this->AgendamentosExamesListaModel->listaDropDownConselhos();
	...
	public function edit()
	{
		...
		if ($this->validation->run($fields) == FALSE) {
			...
			if ($this->AgendamentosExamesListaModel->update($fields['codExameLista'], $fields)) {
	...
	public function removeMembro()
	{
		...
		if (!$this->validation->check($id, 'required|numeric')) {
			...
			if ($this->AgendamentosExamesListaMembroModel->where('codExameListaMembro', $id)->delete()) {
	...
	public function remove()
	{
		...
		if (!$this->validation->check($id, 'required|numeric')) {
			...
			if ($this->AgendamentosExamesListaModel->where('codExameLista', $id)->delete()) {
