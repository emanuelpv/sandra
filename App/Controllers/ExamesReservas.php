...
class ExamesReservas extends BaseController
{

    protected $ExamesReservasModel;
    ...
    public function index()
    {
        ...
        $permissao = verificaPermissao('examesReservas', 'listar');
        if ($permissao == 0) {
            echo mensagemAcessoNegado(session()->organizacoes);
            $this->LogsModel->inserirLog('Acesso indevido ao Módulo "ExamesReservas"', session()->codPessoa);
            exit();
        }


        $data = [
            'controller'        => 'examesReservas',
            'title'             => 'Reservas'
        ];
        echo view('tema/cabecalho');
        echo view('tema/menu_vertical');
        echo view('tema/menu_horizontal');
        return view('examesReservas', $data);
    }
    ...
    public function getAll()
    {
        ...
        $result = $this->ExamesReservasModel->pegaTudo();

        foreach ($result as $key => $value) {

            $ops = '<div class="btn-group">';
            $ops .= '	<button type="button" class="btn btn-sm btn-info"  data-toggle="tooltip" data-placement="top" title="Editar"  onclick="editexamesReservas(' . $value->codExameReserva . ')"><i class="fa fa-edit"></i></button>';
            $ops .= '</div>';
    ...
    public function getOne()
    {
        ...
        if ($this->validation->check($id, 'required|numeric')) {
            ...
            $data = $this->ExamesReservasModel->pegaPorCodigo($id);
    ...
    public function add()
    {
        ...
        $existe = $this->ExamesReservasModel->verificaExistencia($this->request->getPost('codPaciente'), $this->request->getPost('codEspecialidade'));
        ...
        $maisDeX = $this->ExamesReservasModel->verificaExistenciaMaisDeX($this->request->getPost('codPaciente'));
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->ExamesReservasModel->insert($fields)) {
    ...
    public function edit()
    {
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->ExamesReservasModel->update($fields['codExameReserva'], $fields)) {
    ...
    public function remove()
    {
        ...
        if (!$this->validation->check($id, 'required|numeric')) {
            ...
            if ($this->ExamesReservasModel->where('codExameReserva', $id)->delete()) {
