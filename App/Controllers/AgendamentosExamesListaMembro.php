...
class AgendamentosExamesListaMembro extends BaseController
{
	...
	public function __construct()
	{
		...
		$permissao = verificaPermissao('AgendamentosExamesListaMembro', 'listar');
		if ($permissao == 0) {
			echo mensagemAcessoNegado(session()->organizacoes);
			$this->LogsModel->inserirLog('Acesso indevido ao Módulo "AgendamentosExamesListaMembro"', session()->codPessoa);
			exit();
		}
	}
	...
	public function index()
	{
		...
		$data = [
			'controller'    	=> 'agendamentosExamesListaMembro',
			'title'     		=> 'Lista membros'
		];
		echo view('tema/cabecalho');
		echo view('tema/menu_vertical');
		echo view('tema/menu_horizontal');
		return view('agendamentosExamesListaMembro', $data);
	}
	...
	public function getAll()
	{
		...
		$result = $this->AgendamentosExamesListaMembroModel->pegaTudo();

		foreach ($result as $key => $value) {

			$ops = '<div class="btn-group">';
			$ops .= '	<button type="button" class="btn btn-sm btn-info"  data-toggle="tooltip" data-placement="top" title="Editar"  onclick="editagendamentosExamesListaMembro(' . $value->codExameListaMembro . ')"><i class="fa fa-edit"></i></button>';
			$ops .= '	<button type="button" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="top" title="Remover"  onclick="removeagendamentosExamesListaMembro(' . $value->codExameListaMembro . ')"><i class="fa fa-trash"></i></button>';
			$ops .= '</div>';
