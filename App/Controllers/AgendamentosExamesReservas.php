...
class AgendamentosExamesReservas extends BaseController
{

    protected $AgendamentosExamesReservasModel;
    ...
    public function index()
    {
        ...
        $permissao = verificaPermissao('agendamentosExamesReservas', 'listar');
        if ($permissao == 0) {
            echo mensagemAcessoNegado(session()->organizacoes);
            $this->LogsModel->inserirLog('Acesso indevido ao Módulo "AgendamentosExamesReservas"', session()->codPessoa);
            exit();
        }


        $data = [
            'controller'        => 'agendamentosExamesReservas',
            'title'             => 'Reservas'
        ];
        echo view('tema/cabecalho');
        echo view('tema/menu_vertical');
        echo view('tema/menu_horizontal');
        return view('agendamentosExamesReservas', $data);
    }
    ...
    public function getAll()
    {
        ...
        $result = $this->AgendamentosExamesReservasModel->pegaTudo();

        foreach ($result as $key => $value) {

            $ops = '<div class="btn-group">';
            $ops .= '	<button type="button" class="btn btn-sm btn-info"  data-toggle="tooltip" data-placement="top" title="Editar"  onclick="editagendamentosExamesReservas(' . $value->codExameReserva . ')"><i class="fa fa-edit"></i></button>';
            $ops .= '</div>';
    ...
    public function getOne()
    {
        ...
        $id = $this->request->getPost('codExameReserva');

        if ($this->validation->check($id, 'required|numeric')) {

            $data = $this->AgendamentosExamesReservasModel->pegaPorCodigo($id);
    ...
    public function add()
    {
        ...
        $existe = $this->AgendamentosExamesReservasModel->verificaExistencia($this->request->getPost('codPaciente'), $this->request->getPost('codEspecialidade'));
        ...
        $maisDeX = $this->AgendamentosExamesReservasModel->verificaExistenciaMaisDeX($this->request->getPost('codPaciente'));
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->AgendamentosExamesReservasModel->insert($fields)) {
    ...
    public function edit()
    {
        ...
        $fields['codExameReserva'] = $this->request->getPost('codExameReserva');
        ...
        $this->validation->setRules([
            'codExameReserva' => ['label' => 'codExameReserva', 'rules' => 'required|numeric|max_length[11]'],
            'codOrganizacao' => ['label' => 'CodOrganizacao', 'rules' => 'required|numeric|max_length[11]'],
            'codPaciente' => ['label' => 'CodPaciente', 'rules' => 'required|numeric|max_length[11]'],
            'codEspecialidade' => ['label' => 'CodEspecialidade', 'rules' => 'required|numeric|max_length[11]'],
            'codEspecialista' => ['label' => 'CodEspecialista', 'rules' => 'required|numeric|max_length[11]'],
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->AgendamentosExamesReservasModel->update($fields['codExameReserva'], $fields)) {
    ...
    public function remove()
    {
        ...
        if (!$this->validation->check($id, 'required|numeric')) {
            ...
            if ($this->AgendamentosExamesReservasModel->where('codExameReserva', $id)->delete()) {
