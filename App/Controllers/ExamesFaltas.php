...
class ExamesFaltas extends BaseController
{

	protected $ExamesFaltasModel;
	...
	public function index()
	{
		...
		$permissao = verificaPermissao('ExamesFaltas', 'listar');
		if ($permissao == 0) {
			echo mensagemAcessoNegado(session()->organizacoes);
			$this->LogsModel->inserirLog('Acesso indevido ao Módulo "ExamesFaltas"', session()->codPessoa);
			exit();
		}


		$data = [
			'controller'    	=> 'examesFaltas',
			'title'     		=> 'Absenteísmo'
		];
		echo view('tema/cabecalho');
		echo view('tema/menu_vertical');
		echo view('tema/menu_horizontal');
		return view('examesFaltas', $data);
	}
	...
	public function getAll()
	{
		...
		$result = $this->ExamesFaltasModel->pegaTudo();
	...
	public function getOne()
	{
		...
		if ($this->validation->check($id, 'required|numeric')) {
			...
			$data = $this->ExamesFaltasModel->pegaPorCodigo($id);
	...
	public function edit()
	{
		...
		if ($this->validation->run($fields) == FALSE) {
			...
			if ($this->ExamesFaltasModel->update($fields['codExameFalta'], $fields)) {
	...
	public function remove()
	{
		...
		if (!$this->validation->check($id, 'required|numeric')) {
			...
			if ($this->ExamesFaltasModel->where('codExameFalta', $id)->delete()) {
