...
use App\Models\ExamesConfigModel;

class ExamesConfig extends BaseController
{

    protected $ExamesConfigModel;
    ...
    public function __construct()
    {
        ...
        $this->ExamesConfigModel = new ExamesConfigModel();
    ...
    public function listaDropDownStatusExame()
    {
        ...
        $result = $this->ExamesListaModel->listaDropDownStatusExame();
    ...
    public function listaVagasAvulsas()
    {
        ...
        $exames = $this->ExamesModel->cancelamentosExamesPorEspecialidade();


        if ($exames == 'noEspecialidade') {
    ...
    public function listaDropDownTipoExame()
    {
        ...
        $result = $this->ExamesListaModel->listaDropDownTipoExame();
    ...
    public function listaDropDownExamesListaDisponivelMarcacao()
    {
        ...
        $result = $this->ExamesListaModel->listaDropDownExamesListaDisponivelMarcacaoExame();
    ...
    {
        $permissao = verificaPermissao('examesConfig', 'listar');
        if ($permissao == 0) {
            echo mensagemAcessoNegado(session()->organizacoes);
            $this->LogsModel->inserirLog('Acesso indevido ao Módulo "ExamesConfig"', session()->codPessoa);
            exit();
        }

        $data = [
            'controller'        => 'examesConfig',
            'title'             => 'Configuração de Exames'
        ];
        echo view('tema/cabecalho');
        echo view('tema/menu_vertical');
        echo view('tema/menu_horizontal');
        return view('examesConfig', $data);
    }
    ...
    public function getAll()
    {
        ...
        $result = $this->ExamesConfigModel->pegaTudo();

        if ($this->request->getPost('programadas') == 1) {
            $result = $this->ExamesConfigModel->pegaAgendasProgramadas();
        }

        if ($this->request->getPost('liberadasHoje') == 1) {
            $result = $this->ExamesConfigModel->pegaAgendasLiberadasHoje();
        }
        ...
        if ($this->request->getPost('buscaAvancada') == 1) {
            ...
            $result = $this->ExamesConfigModel->buscaAvancada($codEspecialidade,$codEspecialista,$dataInicio,$dataEncerramento);
        }
        ...
        foreach ($result as $key => $value) {
            ...
            $vagasCriadas = $this->ExamesConfigModel->vagasCriadas($value->codConfig)->total;
            $vagasAbertas = $this->ExamesConfigModel->vagasAbertas($value->codConfig)->total;
            ...
            $ops .= '	<button type="button" class="btn btn-sm btn-info"  data-toggle="tooltip" data-placement="top" title="Editar"  onclick="editexamesConfig(' . $value->codConfig . ')"><i class="fa fa-edit"></i></button>';
            $ops .= '	<button type="button" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="top" title="Remover"  onclick="removeexamesConfig(' . $value->codConfig . ')"><i class="fa fa-trash"></i></button>';
            $ops .= '</div>';

            if ($value->codStatusExame == 1) {
                $btnLiberar = '<div><button type="button" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="top" title="Remover"  onclick="mudaStatusExamesConfig(' . $value->codConfig . ',0)"><i class="fa fa-lock"></i>Bloquear</button></div>';
            } else {
                $btnLiberar = '<div><button type="button" class="btn btn-sm btn-success" data-toggle="tooltip" data-placement="top" title="Remover"  onclick="mudaStatusExamesConfig(' . $value->codConfig . ',1)"><i class="fa fa-check"></i>Liberar</button></div>';
            }
    ...
    public function mudaStatusExamesConfig()
    {
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->ExamesConfigModel->update($fields['codConfig'], $fields)) {
    ...
    public function getOne()
    {
        ...
        if ($this->validation->check($id, 'required|numeric')) {
            ...
            $data = $this->ExamesConfigModel->pegaPorCodigo($id);
    ...
    public function add()
    {
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($codConfig = $this->ExamesConfigModel->insert($fields)) {
    ...
    public function remove()
    {
        ...
        if (!$this->validation->check($codConfig, 'required|numeric')) {
            ...
            if ($this->ExamesConfigModel->where('codConfig', $codConfig)->delete()) {
                $this->ExamesModel->removeSlotsNaoAgendados($codConfig);
