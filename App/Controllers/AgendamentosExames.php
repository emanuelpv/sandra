...
use App\Models\AgendamentosExamesReservasModel;

class AgendamentosExames extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->AgendamentosExamesReservasModel = new AgendamentosExamesReservasModel();
    ...
    {
        $permissao = verificaPermissao('AgendamentosExames', 'listar');
        if ($permissao == 0) {
            echo mensagemAcessoNegado(session()->organizacoes);
            $this->LogsModel->inserirLog('Acesso indevido ao Módulo "AgendamentosExames"', session()->codPessoa);
            exit();
        }

        $especialistas = $this->AgendamentosExamesListaModel->especialistas();


        $data = [
            'controller'        => 'agendamentosExames',
            'title'             => 'AgendamentosExames',
        ...
        return view('agendamentosExames', $data);
    }
    ...
    public function consulta()
    {
        ...
        $data = array();
        $data = $this->AgendamentosExamesListaModel->especialistas();

        $data = [
            'controller'        => 'agendamentosExames',
            'title'             => 'Agendamentos Exames',
            'data' => $data,
        ];
        echo view('tema/cabecalho');
        echo view('tema/menu_vertical');
        echo view('tema/menu_horizontal');
        return view('agendamentosExamesConsulta', $data);
    }
    ...
    public function getAll()
    {
        ...
        $result = $this->AgendamentosExamesModel->pegaTudo();


        foreach ($result as $key => $value) {

            $ops = '<div class="btn-group">';
            $ops .= '	<button type="button" class="btn btn-sm btn-info"  data-toggle="tooltip" data-placement="top" title="Editar"  onclick="editagendamentosExames(' . $value->codExame . ')"><i class="fa fa-edit"></i></button>';
            $ops .= '	<button type="button" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="top" title="Remover"  onclick="removeagendamentosExames(' . $value->codExame . ')"><i class="fa fa-trash"></i></button>';
            $ops .= '</div>';
    ...
    public function proximosAgendamentosExames()
    {
        $response = array();
        ...
        $agendamentosExames = $this->AgendamentosExamesModel->proximosAgendamentosExames($codPaciente);
    ...
    public function iniciarExame()
    {
        ...
        if ($codExame !== NULL and $codExame !== "" and $codExame !== " ") {
            if ($this->AgendamentosExamesModel->update($codExame, $fields)) {
                sleep(3);
                $response['success'] = true;
                $response['csrf_hash'] = csrf_hash();
    ...
    public function chegou()
    {
        ...
        $dadosAgendamentosExames = $this->AgendamentosExamesModel->pegaPorCodigo($codExame);

        $especialidade = $dadosAgendamentosExames->descricaoExameLista;
        $especialista = $dadosAgendamentosExames->nomeExibicao;
        $protocolo = $dadosAgendamentosExames->protocolo;
        $dataInicio = date('d/m/Y H:i', strtotime($dadosAgendamentosExames->dataInicio));

        $dadosPaciente = $this->PacientesModel->pegaPacientePorCodPaciente($dadosAgendamentosExames->codPaciente);
    ...
    public function cancelarExame()
    {
        ...
        $dadosExame = $this->AgendamentosExamesModel->pegaPorCodigo($codExame);
        ...
        if (!$this->validation->check($codExame, 'required|numeric')) {
            ...
            $this->AgendamentosExamesModel->update($codExame, $fields);
        }
    ...
    public function marcados()
    {
        ...
        $result = $this->AgendamentosExamesModel->marcados();
    ...
    public function marcadosListaImprimir()
    {
        ...
        foreach ($result as $key => $value) {
            ...
            $outrosContatosPaciente = $this->AgendamentosExamesModel->outrosContatosPorPaciente($value->codPaciente);
    ...
    public function enviarPorEmailDados()
    {
        ...
        $codExame = $this->request->getPost('codExame');
        $exame = $this->AgendamentosExamesModel->comprovante($codExame);
    ...
    public function chamarPacienteAgora()
    {
        ...
        $data = array();
        $codExame = $this->request->getPost('codExame');
        $dados = $this->AgendamentosExamesModel->comprovante($codExame);
    ...
    public function marcarPaciente()
    {
        ...
        $exame = $this->AgendamentosExamesModel->pegaPorCodigo($codExame);
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->AgendamentosExamesModel->update($codExame, $fields)) {
                ...
                if ($codPacienteMarcacao !== NULL and $codPacienteMarcacao !== "" and $codPacienteMarcacao !== " " and $exame->codExameLista !== NULL and $exame->codExameLista !== "" and $exame->codExameLista !== " ") {
                    $this->AgendamentosExamesReservasModel->atualizaStatusReserva($codPacienteMarcacao, $exame->codExameLista);
                }
    ...
    public function agendamentosExamesPorExameLista()
    {
        ...
        $agendamentosExames = $this->AgendamentosExamesModel->agendamentosExamesPorExameLista();


        //VERIFICA SE PERMITE CADASTRO RESERVA
        $verificaCadastroReserva = $this->AgendamentosExamesModel->verificaCadastroReserva($codExameLista);


        if ($agendamentosExames == 'noExameLista') {
        ...
        if (empty($agendamentosExames) or $agendamentosExames == NULL) {
    ...
    <button onclick="entrarListaReservaAgendamentosExames(\'' . $codExameLista . '\',\'' . '\')" type="button" class="btn btn-lg btn-success" data-dismiss="alert" aria-hidden="true"> ENTRAR NA LISTA DE RESERVA</button>
  </div>';
            }
    ...
    public function getOne()
    {
        ...
        if ($this->validation->check($id, 'required|numeric')) {
            ...
            $data = $this->AgendamentosExamesModel->pegaPorCodigo($id);
    ...
    public function add()
    {
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->AgendamentosExamesModel->insert($fields)) {
    ...
    public function reservaUmMinuto()
    {
        ...
        if (!$this->validation->check($fields['codExame'], 'required|numeric')) {
            ...
            $this->AgendamentosExamesModel->update($fields['codExame'], $fields);
        }


        $dadosExame = $this->AgendamentosExamesModel->pegaPorCodigo($fields['codExame']);
    ...
    public function edit()
    {
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->AgendamentosExamesModel->update($fields['codExame'], $fields)) {
    ...
    public function faltou()
    {
        ...
        if (!$this->validation->check($codExame, 'required|numeric')) {
            ...
            if ($this->AgendamentosExamesFaltasModel->insert($fields)) {
                ...
                $this->AgendamentosExamesModel->update($codExame, $statusArray);
                ...
                if ($email !== NULL and $dadosPaciente->nomeExibicao !== NULL) {
                    ...
                    $conteudo .= "<div>Devido à grande procura por agendamentosExames, não será possível remarcar para essa exameLista nos próximos 30 dias.</div>";
                    ...
                    $conteudoSMS = "
                                     Caro senhor(a), " . $nomePaciente . ",";
                    $conteudoSMS .= "Não registramos sua presença no exame " . $exameLista . ". Devido à grande procura por agendamentosExames, não será possível remarcar para essa exameLista nos próximos 30 dias.";
