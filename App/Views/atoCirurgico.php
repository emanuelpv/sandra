...
<!-- Main content -->
          ...
          <div class="card">
            <div class="card-header">
              <div class="row">
				  ...
				  <h3 style="font-size:30px;font-weight: bold;" class="card-title">Ato Cirúrgico</h3>
			  	</div>
				<div class="col-md-4">
				  <button type="button" class="btn btn-block btn-primary" data-toggle="tooltip" data-placement="top" title="Adicionar" onclick="addatoCirurgico()" title="Adicionar">Adicionar</button>
				</div>
			  </div>			  
            </div>
            <!-- /.card-header -->
	...
	<!-- Add modal content -->
	<div id="atoCirurgicoAddModal" class="modal fade" role="dialog"
		aria-hidden="true">
				...
				<div class="modal-body">
					<form id="atoCirurgicoAddForm" class="pl-3 pr-3">
						...
						<div class="row">
							...
							<div class="col-md-4">
								<div class="form-group">
									<label for="codTipoAtoCirurgico"> CodTipoAtoCirurgico: <span class="text-danger">*</span> </label>
	...
	<!-- Add modal content -->				
	<div id="atoCirurgicoEditModal" class="modal fade" role="dialog"
		aria-hidden="true">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
			<div class="modal-header bg-primary text-center p-3">
				<h4 class="modal-title text-white" id="info-header-modalLabel">Atualizar Ato Cirúrgico</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">×</span>
				</button>
			</div>
				<div class="modal-body">
					<form id="atoCirurgicoEditForm" class="pl-3 pr-3">
						<input type="hidden" id="<?php echo csrf_token() ?>atoCirurgicoEditForm" name="<?php echo csrf_token() ?>" value="<?php echo csrf_hash() ?>">
						...
						<div class="form-group text-center">
							<div class="btn-group">
								<button type="submit" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Salvar" id="atoCirurgicoEditForm-btn">Salvar</button>
								<button type="button" class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Fechar" data-dismiss="modal">Fechar</button>
							</div>
						</div>
					</form>
...
$(function () {
	$('#data_tableatoCirurgico').DataTable({
		...
		"responsive": true,
		"ajax": {
			"url": '<?php echo base_url('atoCirurgico/getAll') ?>',			
			"type": "POST",
			"dataType": "json",
			async: "true",
			data: {
...
function editatoCirurgico(codAtoCirurgico) {
	$.ajax({
		...
		success: function(response) {
			// reset the form 
			$("#atoCirurgicoEditForm")[0].reset();
			$(".form-control").removeClass('is-invalid').removeClass('is-valid');				
			$('#atoCirurgicoEditModal').modal('show');	

			$("#atoCirurgicoEditForm #codAtoCirurgico").val(response.codAtoCirurgico);
			$("#atoCirurgicoEditForm #codAtendimento").val(response.codAtendimento);
			$("#atoCirurgicoEditForm #dataCriacao").val(response.dataCriacao);
			$("#atoCirurgicoEditForm #dataAtualizacao").val(response.dataAtualizacao);
			$("#atoCirurgicoEditForm #codAutor").val(response.codAutor);
			$("#atoCirurgicoEditForm #codStatus").val(response.codStatus);
			$("#atoCirurgicoEditForm #codTipoAtoCirurgico").val(response.codTipoAtoCirurgico);
					...
					$.ajax({
						...
						success: function(response) {
							...
							if (response.success === true) {
									...
									$('#atoCirurgicoEditModal').modal('hide');
							...
							$('#atoCirurgicoEditForm-btn').html('Salvar');
						}
					});
