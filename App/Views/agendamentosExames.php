				...
				<div class="card-header">
					<div class="row">
						<div class="col-md-8 mt-2">
							<h3 style="font-size:30px;font-weight: bold;" class="card-title">AgendamentosExames</h3>
						</div>
						...
						<div class="col-12 col-sm-12">
							<div class="card card-primary card-tabs">
								<div class="card-header p-0 pt-1">
									...
									<h3 class="card-title">
										<ul class="nav nav-tabs" id="agendamentosExames-tab" role="tablist">
											<li class="nav-item">
												<a class="nav-link active" id="agendamentosExamesMarcados" data-toggle="pill" href="#agendamentosExames-profile" role="tab" aria-controls="agendamentosExames-profile" aria-selected="true">Agendados</a>
											</li>
											<li class="nav-item">
												<a class="nav-link" id="agendamentosExamesLivres" data-toggle="pill" href="#agendamentosExames-home" role="tab" aria-controls="agendamentosExames-home" aria-selected="false">Vagas livres</a>
											</li>
								...
								<div class="card-body">
									<div class="tab-content" id="agendamentosExames-tabContent">
										<div class="tab-pane fade" id="agendamentosExames-home" role="tabpanel" aria-labelledby="agendamentosExamesLivres">

											<div id="slotsLivres"> </div>
										</div>
										<div class="tab-pane fade  show active" id="agendamentosExames-profile" role="tabpanel" aria-labelledby="agendamentosExamesMarcados">
...
<div id="agendamentosExamesReservasAddModal" class="modal fade" role="dialog" aria-hidden="true">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header bg-primary text-center p-3">
			...
			<div class="modal-body">
				...
				<form id="agendamentosExamesReservasAddForm" class="pl-3 pr-3">
					<div class="row">
						<input type="hidden" id="<?php echo csrf_token() ?>agendamentosExamesReservasAddForm" name="<?php echo csrf_token() ?>" value="<?php echo csrf_hash() ?>">
					...
					<div class="form-group text-center">
						<div class="btn-group">
							<button type="button" onclick="salvarReserva()" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Adicionar" id="agendamentosExamesReservasAddForm-btn">Salvar Reserva</button>
							<button type="button" class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Fechar" data-dismiss="modal">Fechar</button>
						</div>
					</div>
				</form>
...
<script>
	...
	function entrarListaReservaAgendamentosExames(exameLista, especialista) {
	...
	$(function() {
		...
		$.ajax({
			...
			success: function(agendamentosExamesListaAdm) {

				$("#codExameListaAdd").select2({
					data: agendamentosExamesListaAdm,
				})
	...
	function selecionarPaciente() {
		...
		$.ajax({
			url: '<?php echo base_url('agendamentosExames/nomePaciente') ?>',
			type: 'post',
			data: {
	...
	function gravarSala() {
		...
		$.ajax({
			url: '<?php echo base_url('agendamentosExames/gravarSala') ?>',
			type: 'post',
			data: {
	...
	function procurarPacienteReserva() {
		...
		$.ajax({
			url: '<?php echo base_url('agendamentosExames/procurarPaciente') ?>',
			type: 'post',
			data: form.serialize(), // /converting the form data into array and sending it to server
			dataType: 'json',
			success: function(procuraPaciente) {
	...
	function addagendamentosExames() {
		// reset the form 
		$("#agendamentosExamesAddForm")[0].reset();
		$(".form-control").removeClass('is-invalid').removeClass('is-valid');
		$('#agendamentosExamesAddModal').modal('show');
		// submit the add from 
		$.validator.setDefaults({
			...
			submitHandler: function(form) {
				...
				var form = $('#agendamentosExamesAddForm');
				// remove the text-danger
				$(".text-danger").remove();

				$.ajax({
					url: '<?php echo base_url('agendamentosExames/add') ?>',
					type: 'post',
					data: form.serialize(), // /converting the form data into array and sending it to server
					dataType: 'json',
					beforeSend: function() {
						//$('#agendamentosExamesAddForm-btn').html('<i class="fa fa-spinner fa-spin"></i>');
					},
					success: function(response) {

						if (response.success === true) {
							$('#agendamentosExamesAddModal').modal('hide');
						...
						$('#agendamentosExamesAddForm-btn').html('Adicionar');
					}
				});
		...
		$('#agendamentosExamesAddForm').validate();
	}

	function editagendamentosExames(codExame) {
		$.ajax({
			url: '<?php echo base_url('agendamentosExames/getOne') ?>',
			type: 'post',
			data: {
				codExame: codExame,
				csrf_sandra: $("#csrf_sandraPrincipal").val(),
			...
			success: function(response) {
				// reset the form 
				$("#agendamentosExamesEditForm")[0].reset();
				$(".form-control").removeClass('is-invalid').removeClass('is-valid');
				$('#agendamentosExamesEditModal').modal('show');

				$("#agendamentosExamesEditForm #codExame").val(response.codExame);
				$("#agendamentosExamesEditForm #codConfig").val(response.codConfig);
				$("#agendamentosExamesEditForm #codOrganizacao").val(response.codOrganizacao);
				$("#agendamentosExamesEditForm #codPaciente").val(response.codPaciente);
				$("#agendamentosExamesEditForm #codLocal").val(response.codLocal);
				$("#agendamentosExamesEditForm #codEspecialista").val(response.codEspecialista);
				$("#agendamentosExamesEditForm #codExameLista").val(response.codExameLista);
				$("#agendamentosExamesEditForm #codStatus").val(response.codStatus);
				$("#agendamentosExamesEditForm #dataCriacao").val(response.dataCriacao);
				$("#agendamentosExamesEditForm #dataAtualizacao").val(response.dataAtualizacao);
				$("#agendamentosExamesEditForm #dataInicio").val(response.dataInicio);
				$("#agendamentosExamesEditForm #dataEncerramento").val(response.dataEncerramento);
				$("#agendamentosExamesEditForm #codTipoExame").val(response.codTipoExame);
				$("#agendamentosExamesEditForm #codAutor").val(response.codAutor);
				$("#agendamentosExamesEditForm #protocolo").val(response.protocolo);
				$("#agendamentosExamesEditForm #ordemAtendimento").val(response.ordemAtendimento);
				...
				$.validator.setDefaults({
					...
					submitHandler: function(form) {
						var form = $('#agendamentosExamesEditForm');
						$(".text-danger").remove();
						$.ajax({
							url: '<?php echo base_url('agendamentosExames/edit') ?>',
							type: 'post',
							data: form.serialize(),
							dataType: 'json',
							beforeSend: function() {
								//$('#agendamentosExamesEditForm-btn').html('<i class="fa fa-spinner fa-spin"></i>');
							},
							success: function(response) {

								if (response.success === true) {

									$('#agendamentosExamesEditModal').modal('hide');
								...
								$('#agendamentosExamesEditForm-btn').html('Salvar');
							}
						});
				...
				$('#agendamentosExamesEditForm').validate();

			}
		});
	}

	function removeagendamentosExames(codExame) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: '<?php echo base_url('agendamentosExames/remove') ?>',
					type: 'post',
					data: {
						codExame: codExame,
						csrf_sandra: $("#csrf_sandraPrincipal").val(),
	...
	function desmarcar(codExame) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: '<?php echo base_url('agendamentosExames/desmarcar') ?>',
					type: 'post',
					data: {
						codExame: codExame,
						csrf_sandra: $("#csrf_sandraPrincipal").val(),
	...
	function confirmou(codExame) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: '<?php echo base_url('agendamentosExames/confirmou') ?>',
					type: 'post',
					data: {
						codExame: codExame,
						csrf_sandra: $("#csrf_sandraPrincipal").val(),
