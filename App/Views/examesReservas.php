	...
	<div class="row">
		<div class="col-12">
			<div class="card">
				...
				<!-- /.card-header -->
				<div class="card-body">
					<table id="data_tableexamesReservas" class="table table-striped table-hover table-sm">
						<thead>
							<tr>
								<th>Protocolo</th>
								<th>Paciente</th>
...
<!-- Add modal content -->
<div id="examesReservasEditModal" class="modal fade" role="dialog" aria-hidden="true">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			...
			<div style="font-size:16px" class="modal-body">
				<form id="examesReservasEditForm" class="pl-3 pr-3">
					<div class="row">
						<input type="hidden" id="<?php echo csrf_token() ?>examesReservasEditForm" name="<?php echo csrf_token() ?>" value="<?php echo csrf_hash() ?>">
...
<script>
	...
	$(function() {
		$('#data_tableexamesReservas').DataTable({
			"paging": true,
			"deferRender": true,
			"lengthChange": false,
			"searching": true,
			...
			"responsive": true,
			"ajax": {
				"url": '<?php echo base_url('examesReservas/getAll') ?>',
				"type": "POST",
				"dataType": "json",
				async: "true",
				data: {
	...
	function addexamesReservas() {
		...
		$.validator.setDefaults({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					...
					dataType: 'json',
					beforeSend: function() {
						//$('#examesReservasAddForm-btn').html('<i class="fa fa-spinner fa-spin"></i>');
					},
					success: function(response) {
						...
						if (response.success === true) {
							...
							Toast.fire({
								...
								$('#data_tableexamesReservas').DataTable().ajax.reload(null, false).draw(false);
							})
						...
						$('#examesReservasAddForm-btn').html('Adicionar');
					}
				});
		...
		$('#examesReservasAddForm').validate();
	}
	...
	function escolhaPaciente(codExame) {
		...
		var codPaciente = $("#examesReservasEditForm #codPaciente").val();
					...
					if (result.value) {
						$.ajax({
							...
							success: function(marcacaoPaciente) {
								...
								$('#examesReservasEditModal').modal('hide');
	...
	function buscarVagas() {
		...
		var form = $('#examesReservasEditForm');
		$.ajax({
	...
	function editexamesReservas(codExameReserva) {
		$.ajax({
			url: '<?php echo base_url('examesReservas/getOne') ?>',
			type: 'post',
			data: {
			...
			success: function(response) {
				// reset the form 
				$("#examesReservasEditForm")[0].reset();
				$(".form-control").removeClass('is-invalid').removeClass('is-valid');
				$('#examesReservasEditModal').modal('show');

				document.getElementById('slotsLivres').innerHTML = '';


				$("#examesReservasEditForm #codExameReserva").val(response.codExameReserva);
				$("#examesReservasEditForm #codOrganizacao").val(response.codOrganizacao);
				$("#examesReservasEditForm #codPaciente").val(response.codPaciente);
				$("#examesReservasEditForm #codEspecialidade").val(response.codEspecialidade);
				$("#examesReservasEditForm #codEspecialista").val(response.codEspecialista);
				$("#examesReservasEditForm #codStatus").val(response.codStatus);
				$("#examesReservasEditForm #dataCriacao").val(response.dataCriacao);
				$("#examesReservasEditForm #dataAtualizacao").val(response.dataAtualizacao);
				$("#examesReservasEditForm #preferenciaDia").val(response.preferenciaDia);
				$("#examesReservasEditForm #preferenciaHora").val(response.preferenciaHora);
				$("#examesReservasEditForm #codAutor").val(response.codAutor);
				$("#examesReservasEditForm #protocolo").val(response.protocolo);

				if (response.codStatus !== '2') {

					document.getElementById('btn-buscarVagas').innerHTML = '<button type="button" onclick="buscarVagas()" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Salvar" id="examesReservasEditForm-btn">BUSCAR VAGAS</button>';
				...
				$.validator.setDefaults({
					...
					submitHandler: function(form) {
						...
						$(".text-danger").remove();
						$.ajax({
							url: '<?php echo base_url('examesReservas/edit') ?>',
							type: 'post',
							data: form.serialize(),
							dataType: 'json',
							beforeSend: function() {
								//$('#examesReservasEditForm-btn').html('<i class="fa fa-spinner fa-spin"></i>');
							},
							success: function(response) {
								...
								$('#examesReservasEditForm-btn').html('Salvar');
							}
						});
				...
				$('#examesReservasEditForm').validate();

			}
		});
	}

	function removeexamesReservas(codExameReserva) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: '<?php echo base_url('examesReservas/remove') ?>',
					type: 'post',
					data: {
						codExameReserva: codExameReserva,
						csrf_sandra: $("#csrf_sandraPrincipal").val(),
