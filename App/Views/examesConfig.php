				...
				<div class="card-header">
					<div class="row">
						<div class="col-md-4 mt-2">
							<h3 style="font-size:30px;font-weight: bold;" class="card-title">Configuração de Exames</h3>
						</div>
						<div class="col-md-4">
							<button type="button" class="btn btn-block btn-primary" data-toggle="tooltip" data-placement="top" title="Adicionar" onclick="addexamesConfig()" title="Adicionar"> <i class="fa fa-plus"></i> Adicionar</button>
						</div>

						<div class="col-md-4">
							<button type="button" class="btn btn-block btn-danger" data-toggle="tooltip" data-placement="top" title="Remover Vagas" onclick="cancelarExamesConfig()"> <i class="fa fa-plus"></i>Cancelar vagas</button>
						</div>
					</div>
				</div>
				<!-- /.card-header -->
							...
							<div class="card-body">
								<div class="tab-content" id="aba-tabContent">
									<div class="tab-pane fade show active" id="aba-agandasPendentes" role="tabpanel" aria-labelledby="aba-agandasPendentes-tab">
										<table id="data_tableexamesConfig" class="table table-striped table-hover table-sm">
											<thead>
												<tr>
													<th>Código</th>
													<th>ExameLista</th>
									...
									<div class="tab-pane fade" id="aba-liberadasHoje" role="tabpanel" aria-labelledby="aba-liberadasHoje-tab">
										<table id="data_tableexamesConfigLiberadasHoje" class="table table-striped table-hover table-sm">
											<thead>
												<tr>
													<th>Código</th>
													<th>ExameLista</th>
									...
									<div class="tab-pane fade" id="aba-buscaAvancada" role="tabpanel" aria-labelledby="aba-buscaAvancada-tab">
										...
										<div class="row">
											...
											<table id="data_tableexamesConfigBuscaAvancada" class="table table-striped table-hover table-sm">
												<thead>
													<tr>
														<th>Código</th>
														<th>ExameLista</th>
...
<!-- Add modal content -->
<div id="add-modalexamesConfig" class="modal fade" role="dialog" aria-hidden="true">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header bg-primary text-center p-3">
				<h4 class="modal-title text-white" id="info-header-modalLabel">Adicionar Configuração de Exames</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">×</span>
				</button>
			</div>
			<div class="modal-body">
				<form id="add-formexamesConfig" class="pl-3 pr-3">
					<div class="row">
						<input type="hidden" id="<?php echo csrf_token() ?>add-formexamesConfig" name="<?php echo csrf_token() ?>" value="<?php echo csrf_hash() ?>">
					...
					<div class="form-group text-center">
						<div class="btn-group">
							<button type="submit" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Adicionar" id="add-formexamesConfig-btn">Adicionar</button>
...
<div id="cancelar-modalexamesConfig" class="modal fade" role="dialog" aria-hidden="true">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header bg-primary text-center p-3">
			...
			<div class="modal-body">
				<form id="cancelar-formexamesConfig" class="pl-3 pr-3">
					<div class="row">
						<input type="hidden" id="<?php echo csrf_token() ?>cancelar-formexamesConfig" name="<?php echo csrf_token() ?>" value="<?php echo csrf_hash() ?>">
...
<!-- Add modal content -->
<div id="edit-modalexamesConfig" class="modal fade" role="dialog" aria-hidden="true">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header bg-primary text-center p-3">
				<h4 class="modal-title text-white" id="info-header-modalLabel">Configuração de Exames</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">×</span>
				</button>
			</div>
			<div class="modal-body">
				...
				<form id="edit-formexamesConfig" class="pl-3 pr-3">
					<div class="row">
						<input type="hidden" id="<?php echo csrf_token() ?>edit-formexamesConfig" name="<?php echo csrf_token() ?>" value="<?php echo csrf_hash() ?>">
					...
					<div class="form-group text-center">
						<div class="btn-group">
							<button type="submit" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Adicionar" id="add-formexamesConfig-btn">Salvar</button>
							<button type="button" class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Fechar" data-dismiss="modal">Fechar</button>
						</div>
					</div>
				</form>
...
<script>
	...
	$(function() {
		$('#data_tableexamesConfig').DataTable({
			"paging": true,
			"pageLength": 100,
			"lengthChange": false,
			"searching": true,
			...
			"responsive": true,
			"ajax": {
				"url": '<?php echo base_url('examesConfig/getAll') ?>',
				"type": "POST",
				"dataType": "json",
				async: "true",
				data: {
		...
		$('#data_tableexamesConfigLiberadasHoje').DataTable({
			"paging": true,
			"lengthChange": false,
			"pageLength": 100,
			"searching": true,
		...
		$.ajax({
			url: '<?php echo base_url('examesConfig/listaDropDownExamesListaDisponivelMarcacao') ?>',
			type: 'post',
			dataType: 'json',
			data: {
				csrf_sandra: $("#csrf_sandraPrincipal").val(),
			},
			success: function(examesConfigAdd) {

				$("#codEspecialidadeBuscaAvancada").select2({
					data: examesConfigAdd,
				})
		...
		$("#codEspecialidadeBuscaAvancada").on("change", function() {
			...
			$.ajax({
				url: '<?php echo base_url('examesConfig/listaDropDownEspecialistasDisponivelMarcacao') ?>',
				type: 'post',
				dataType: 'json',
				data: {
	...
	function mudaStatusExamesConfig(codConfig, codStatusExame) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: '<?php echo base_url('examesConfig/mudaStatusExamesConfig') ?>',
					type: 'post',
					data: {
						codConfig: codConfig,
					...
					success: function(response) {
						...
						if (response.success === true) {
							...
							Toast.fire({
								...
								$('#data_tableexamesConfig').DataTable().ajax.reload(null, false).draw(false);
								$('#data_tableexamesConfigLiberadasHoje').DataTable().ajax.reload(null, false).draw(false);
								if ($.fn.DataTable.isDataTable('#data_tableexamesConfigBuscaAvancada')) {
									$('#data_tableexamesConfigBuscaAvancada').DataTable().ajax.reload(null, false).draw(false);
								}
							})
						} else {
							Swal.fire({
	...
	function buscarAgora() {
		...
		$('#data_tableexamesConfigBuscaAvancada').DataTable({
			"bDestroy": true,
			"paging": true,
			"pageLength": 100,
			"lengthChange": false,
	...
	function addexamesConfig() {
		// reset the form 
		$("#add-formexamesConfig")[0].reset();
		$(".form-control").removeClass('is-invalid').removeClass('is-valid');
		$('#add-modalexamesConfig').modal('show');


		$.ajax({
			url: '<?php echo base_url('examesConfig/listaDropDownTipoExame') ?>',
			type: 'post',
			dataType: 'json',
			data: {
				csrf_sandra: $("#csrf_sandraPrincipal").val(),
		...
		$.ajax({
			url: '<?php echo base_url('examesConfig/listaDropDownStatusExame') ?>',
			type: 'post',
			dataType: 'json',
			data: {
				csrf_sandra: $("#csrf_sandraPrincipal").val(),
		...
		$.validator.setDefaults({
			...
			submitHandler: function(form) {
				...
				var form = $('#add-formexamesConfig');
				// remove the text-danger
				$(".text-danger").remove();

				$.ajax({
					url: '<?php echo base_url('examesConfig/add') ?>',
					type: 'post',
					...
					success: function(response) {
						...
						if (response.success === true) {
							...
							$('#add-modalexamesConfig').modal('hide');
							$('#data_tableexamesConfig').DataTable().ajax.reload(null, false).draw(false);
							$('#data_tableexamesConfigLiberadasHoje').DataTable().ajax.reload(null, false).draw(false);
						...
						$('#add-formexamesConfig-btn').html('Adicionar');
					}
				})
	...
	function cancelarExame(codExame) {
		...
		Swal.fire({
			...
			if (result.value) {
				...
				$.ajax({
					url: '<?php echo base_url('exames/cancelarExame') ?>',
					type: 'post',
					dataType: 'json',
					data: {
						codExame: codExame,
					...
					success: function(cancelarExameResultado) {
						...
						var form = $('#cancelar-formexamesConfig');

						$.ajax({
							url: '<?php echo base_url('examesConfig/listaVagasAvulsas') ?>',
							type: 'post',
							data: form.serialize(), // /converting the form data into array and sending it to server
							dataType: 'json',
	...
	function cancelarExamesConfig() {
		// reset the form 

		$("#cancelar-formexamesConfig")[0].reset();
		$(".form-control").removeClass('is-invalid').removeClass('is-valid');
		$('#cancelar-modalexamesConfig').modal('show');
		...
		$.ajax({
			...
			success: function(examesConfigCancelar) {

				$("#codEspecialidadeCancelar").select2({
					data: examesConfigCancelar,
				})
		...
		$('#cancelar-formexamesConfig').validate();
	}



	function editexamesConfig(codConfig) {
		$.ajax({
			url: '<?php echo base_url('examesConfig/getOne') ?>',
			type: 'post',
			data: {
				codConfig: codConfig,
				csrf_sandra: $("#csrf_sandraPrincipal").val(),
			...
			success: function(response) {
				// reset the form 
				$("#edit-formexamesConfig")[0].reset();
				$(".form-control").removeClass('is-invalid').removeClass('is-valid');
				$('#edit-modalexamesConfig').modal('show');

				$("#edit-formexamesConfig #codConfigEdit").val(response.codConfig);
				$("#edit-formexamesConfig #codEspecialistaEdit").val(response.codEspecialista);
				$("#edit-formexamesConfig #dataCriacaoEdit").val(response.dataCriacao);
				$("#edit-formexamesConfig #dataInicioEdit").val(response.dataInicio);
				$("#edit-formexamesConfig #horaInicioEdit").val(response.horaInicio);
				$("#edit-formexamesConfig #dataEncerramentoEdit").val(response.dataEncerramento);
				$("#edit-formexamesConfig #horaEncerramentoEdit").val(response.horaEncerramento);
				$("#edit-formexamesConfig #tempoAtendimentoEdit").val(response.tempoAtendimento);
				$("#edit-formexamesConfig #intervaloAtendimentoEdit").val(response.intervaloAtendimento);
				$("#edit-formexamesConfig #dataAtualizacaoEdit").val(response.dataAtualizacao);
				...
				$.ajax({
					...
					success: function(examesConfigEdit) {

						$("#codExameListaEdit").select2({
							data: examesConfigEdit,
						})
				...
				$.validator.setDefaults({
					...
					submitHandler: function(form) {
						var form = $('#edit-formexamesConfig');
						$(".text-danger").remove();
						$.ajax({
							url: '<?php echo base_url('examesConfig/edit') ?>',
							type: 'post',
							data: form.serialize(),
							dataType: 'json',
							beforeSend: function() {
								//$('#edit-formexamesConfig-btn').html('<i class="fa fa-spinner fa-spin"></i>');
							},
							success: function(response) {

								if (response.success === true) {

									$('#edit-modalexamesConfig').modal('hide');
								...
								$('#edit-formexamesConfig-btn').html('Salvar');
							}
						});
				...
				$('#edit-formexamesConfig').validate();

			}
		});
	}

	function removeexamesConfig(codConfig) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: '<?php echo base_url('examesConfig/remove') ?>',
					type: 'post',
					data: {
						codConfig: codConfig,
						csrf_sandra: $("#csrf_sandraPrincipal").val(),
