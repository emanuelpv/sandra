               ...
               with font-awesome or any other icon font library -->
                ...
                ?>
                        <li class="nav-item">
                            ...
                            ?>
                            <a href="#" onclick="editPacienteLogado(<?php echo session()->codPaciente ?>)" class="nav-link">
                                <i class="far fa-address-card nav-icon"></i>
                                <p>Perfil</p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="#" onclick="trocasenhaPaciente(<?php echo session()->codPaciente ?>)" class="nav-link">
                    ...
                    ?>
                        <li class="nav-item">
                            ...
                            ?>
                            <a href="#" onclick="editPessoaLogada(<?php echo session()->codPessoa ?>)" class="nav-link">
                                <i class="far fa-address-card nav-icon"></i>
                                <p>Perfil</p>
                            </a>
                        </li>
                        ...
                        <li class="nav-item">
                            <a href="<?php echo base_url('veiculos') ?>" class="nav-link">
                        ...
                        <li class="nav-item">
                            <a href="#" onclick="trocasenhaPessoaLogada()" class="nav-link">
                                <i class="fas fa-user-lock nav-icon"></i>
                                <p>Troca Senha</p>
                            </a>
                        </li>
                ...
                <li class="nav-item">
                    <a href="<?php echo base_url() . '/login/logout/?autorizacao=' . session()->autorizacao ?>" class="nav-link">
    ...
    $(function() {
        ...
        $.fn.dataTable.ext.errMode = () =>
            Swal.fire({
                ...
                html: '<div  class="row text-left">1) Tente o comando novamente;</div><div class="row text-left">2) Tente recarregue o sistema com F5;</div><div class="row text-left">3) Saia do sistema e entre novamente;</div><div class="row text-left">4) O sistema pode estar passando por instavilidade, tente novamente mais tarde; e</div><div class="row text-left">5) Entre em contato com a equipe de Suporte.</div>',
        ...
        $(document).ajaxError(function myErrorHandler(event, xhr, ajaxOptions, thrownError) {
            if ((xhr.status !== 404 && xhr.status !== 200)) { // || xhr.status == 'NaN' || xhr.status == 0
                ...
                Swal.fire({
                    icon: 'warning',
                    title: 'Não foi possível processar a requsição!',
                    html: '<div  class="row text-left">1) Tente o comando novamente;</div><div class="row text-left">2) Tente recarregue o sistema com F5;</div><div class="row text-left">3) Saia do sistema e entre novamente;</div><div class="row text-left">4) O sistema pode estar passando por instavilidade, tente novamente mais tarde; e</div><div class="row text-left">5) Entre em contato com a equipe de Suporte.</div><div>' + xhr.status + ': ' + message + '</div>',
                    showConfirmButton: true,
                    confirmButtonText: 'Ok',
                    ...
                    if (result['isConfirmed']) {
                        $.ajax({
                            url: '<?php echo base_url('verificalogin/verificaSeLogado') ?>',
                            type: 'post',
                            dataType: 'json',
                            data: {
                                csrf_sandra: $("#csrf_sandraPrincipal").val(),
                            },
                            success: function(verificacao) {
                                if (verificacao.success === true) {} else {
                                    Swal.fire({
                                        icon: 'info',
                                        title: 'Identificamos que a sua sessão expirou!',
                                        showConfirmButton: true,
                                        confirmButtonText: 'Ok',
                                        allowOutsideClick: false,
                                        allowEscapeKey: false
