					...
					<div class="row">
						...
						<div class="col-md-12">
							...
							<div class="form-group  text-right">
								<div class="btn-group">
									<button type="button" class="btn btn-xs btn-primary" data-toggle="tooltip" data-placement="top" title="Adicionar Departamento" onclick="add()" title="Adicionar"> <i class="fa fa-plus"></i> Adicionar</button>
									<button type="button" class="btn btn-xs btn-primary" data-toggle="tooltip" data-placement="top" title="Importar Departamentos" onclick="ImportarDepartamentos()" title="Importar Departamentos"> <i class="fas fa-file-import"></i> Importar</button>
									<button type="button" class="btn btn-xs btn-primary" data-toggle="tooltip" data-placement="top" title="Exportar Departamentos" onclick="ExportarDepartamentos()" title="Exportar Departamentos"> <i class="fa fa-file-export"></i> Exportar</button>
									<button type="button" class="btn btn-xs btn-primary" data-toggle="tooltip" data-placement="top" title="Mostrar Logs" onclick="mostraLogs()" title="Logs"> <i class="fas fa-exclamation-triangle"></i> Logs</button>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="col-md-4">
							<div class="form-group">
								...
								<select id="codTipoDepartamento" name="codTipoDepartamento" class="custom-select">
									...
									<option value="5">Oncológico</option>
									<option value="6">Internações OCS</option>
								</select>
							</div>
						</div>
					</div>
					...
					<div class="card card-primary card-tabs">
						<div class="card-header p-0 pt-1">
							<ul class="nav nav-tabs" id="aba-tab" role="tablist">
								...
								<li class="nav-item">
									<a class="nav-link" id="aba-funcoes-tab" data-toggle="pill" href="#aba-funcoes" role="tab" aria-controls="aba-funcoes" aria-selected="false">Funções</a>
								</li>
							</ul>
						</div>
						<div class="card-body">
							<div class="tab-content" id="aba-tabContent">
									...
									<form id="edit-form" class="pl-3 pr-3">
										...
										<div class="row">
											...
											<div class="col-md-4">
												<div class="form-group">
													<label for="paiDepartamento"> Subordinação: <span class="text-danger">*</span> </label>
													<select id="paiDepartamentoEdit" name="paiDepartamento" class="custom-select">
														<option value=""></option>
													</select>
												</div>
											</div>
								...
								<div class="tab-pane fade" id="aba-membros" role="tabpanel" aria-labelledby="aba-membros-tab">
									...
									<div class="row">
										<div class="col-md-4">
											<div class="form-group">
												<label for="codChefe"> Chefe: <span class="text-danger">*</span> </label>
												<select id="codChefe" name="codChefe" class="custom-select">
													<option></option>
												</select>
											</div>
										</div>
								...
								<div class="tab-pane fade" id="aba-funcoes" role="tabpanel" aria-labelledby="aba-funcoes-tab">
									Funções
...
<script>
	...
	function edit(codDepartamento) {
		document.getElementById("codChefe").value = null;
		...
		$.ajax({
			...
			success: function(response) {
				...
				$("#edit-form #paiDepartamento").val(response.paiDepartamento);
				...
				$("#edit-form #codChefe").val(response.codChefe);
				$("#edit-form #codTipoDepartamento").val(response.codTipoDepartamento);
				...
				$('#codChefe').on('change', function() {
					if ($('#codChefe').val() !== "" && $('#codChefe').val() !== response.codChefe) {


						$.ajax({
							url: '<?php echo base_url('departamentos/defineChefe') ?>',
							type: 'post',
							dataType: 'json',
							data: {
								codChefe: $('#codChefe').val(),
								codDepartamento: codDepartamento,
								csrf_sandra: $("#csrf_sandraPrincipal").val(),
							},
							success: function(response) {
				...
				$.ajax({
					...
					success: function(chefe) {

						$("#codChefe").select2({
							data: chefe,
						})

						$('#codChefe').val(response.codChefe); // Select the option with a value of '1'
						$('#codChefe').trigger('change');
						$(document).on('select2:open', () => {
							document.querySelector('.select2-search__field').focus();
						});
				...
				$('#data_tableMembros').DataTable({
					...
					"responsive": true,
					"ajax": {
						"url": '<?php echo base_url($controller . '/membrosDepartamento') ?>',
						"type": "post",
						"dataType": "json",
						data: {
							codDepartamento: codDepartamento,
					...
					success: function(responsedata_tableMembros) {
					...
					submitHandler: function(form) {
						...
						$.ajax({
							...
							success: function(response) {
								...
								if (response.success === true) {
									$("input[id*='csrf_sandra']").val(response.csrf_hash);
									if (response.csrf_hash !== undefined && response.csrf_hash !== null && response.csrf_hash !== ' ') {
