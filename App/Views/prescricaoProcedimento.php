				...
				<div class="card-header">
					<div class="row">
						<div class="col-md-8 mt-2">
							<h3 style="font-size:30px;font-weight: bold;" class="card-title">Prescrição de Procedimentos</h3>
						</div>
						<div class="col-md-4">
							<button type="button" class="btn btn-block btn-primary" data-toggle="tooltip" data-placement="top" title="Adicionar" onclick="addprescricaoProcedimento()" title="Adicionar">Adicionar</button>
						</div>
					</div>
				</div>
				<!-- /.card-header -->
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="prescricaoProcedimentoAddForm" class="pl-3 pr-3">
					...
					<div class="row">
						<input type="hidden" id="codPrescricaoProcedimento" name="codPrescricaoProcedimento" class="form-control" placeholder="Codigo" maxlength="11" required>
					</div>
					<div class="row">
						<div class="col-md-4">
...
<script>
	...
	$(function() {
		$('#data_tableprescricaoProcedimento').DataTable({
			...
			"responsive": true,
			"ajax": {
				"url": '<?php echo base_url('PrescricaoProcedimentos/getAll') ?>',
				"type": "POST",
				"dataType": "json",
				async: "true",
				data: {
	...
	function editprescricaoProcedimento(codPrescricaoProcedimento) {
		$.ajax({
			...
			success: function(response) {
				...
				$("#prescricaoProcedimentoEditForm #codPrescricaoProcedimento").val(response.codPrescricaoProcedimento);
				$("#prescricaoProcedimentoEditForm #codAtendimentoPrescricao").val(response.codAtendimentoPrescricao);
				$("#prescricaoProcedimentoEditForm #codProcedimento").val(response.codProcedimento);
				...
				$("#prescricaoProcedimentoEditForm #codAutor").val(response.codAutor);
				$("#prescricaoProcedimentoEditForm #dataCriacao").val(response.dataCriacao);
				$("#prescricaoProcedimentoEditForm #dataAtualizacao").val(response.dataAtualizacao);
