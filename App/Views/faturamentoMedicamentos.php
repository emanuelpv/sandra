...
<!-- Main content -->
          ...
          <div class="card">
            <div class="card-header">
              <div class="row">
				  ...
				  <h3 style="font-size:30px;font-weight: bold;" class="card-title">Faturamento de Medicamentos</h3>
			  	</div>
				<div class="col-md-4">
				  <button type="button" class="btn btn-block btn-primary" data-toggle="tooltip" data-placement="top" title="Adicionar" onclick="addfaturamentoMedicamentos()" title="Adicionar">Adicionar</button>
				</div>
			  </div>			  
            </div>
            <!-- /.card-header -->
	...
	<!-- Add modal content -->
	<div id="faturamentoMedicamentosAddModal" class="modal fade" role="dialog"
		aria-hidden="true">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
			<div class="modal-header bg-primary text-center p-3">
				<h4 class="modal-title text-white" id="info-header-modalLabel">Adicionar Faturamento de Medicamentos</h4>
				...
				<div class="modal-body">
					<form id="faturamentoMedicamentosAddForm" class="pl-3 pr-3">
						...
						<div class="row">
							...
							<div class="col-md-4">
								<div class="form-group">
									<label for="codPrescricaoMedicamento"> CodPrescricaoMedicamento: <span class="text-danger">*</span> </label>
									<input type="number" id="codPrescricaoMedicamento" name="codPrescricaoMedicamento" class="form-control" placeholder="CodPrescricaoMedicamento" maxlength="11" number="true" required>
								</div>
							</div>
							<div class="col-md-4">
								<div class="form-group">
	...
	<!-- Add modal content -->				
	<div id="faturamentoMedicamentosEditModal" class="modal fade" role="dialog"
		aria-hidden="true">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
			<div class="modal-header bg-primary text-center p-3">
				...
				<div class="modal-body">
					<form id="faturamentoMedicamentosEditForm" class="pl-3 pr-3">
						...
						<div class="form-group text-center">
							<div class="btn-group">
								<button type="submit" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Salvar" id="faturamentoMedicamentosEditForm-btn">Salvar</button>
								<button type="button" class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Fechar" data-dismiss="modal">Fechar</button>
							</div>
						</div>
					</form>
...
$(function () {
	$('#data_tablefaturamentoMedicamentos').DataTable({
		...
		"responsive": true,
		"ajax": {
			"url": '<?php echo base_url('faturamentoMedicamentos/getAll') ?>',			
			"type": "POST",
			"dataType": "json",
			async: "true",
			data: {
...
function addfaturamentoMedicamentos() {
	// reset the form 
	...
	$.validator.setDefaults({
		...
		submitHandler: function(form) {
			...
			$.ajax({
				url: '<?php echo base_url('faturamentoMedicamentos/add') ?>',						
				type: 'post',
...
function editfaturamentoMedicamentos(codFaturamentoMedicamento) {
	$.ajax({
		...
		success: function(response) {
			...
			$("#faturamentoMedicamentosEditForm #codAtendimento").val(response.codAtendimento);
			$("#faturamentoMedicamentosEditForm #codPrescricaoMedicamento").val(response.codPrescricaoMedicamento);
			$("#faturamentoMedicamentosEditForm #autorPrescricao").val(response.autorPrescricao);
			$("#faturamentoMedicamentosEditForm #dataPrescricao").val(response.dataPrescricao);
			$("#faturamentoMedicamentosEditForm #codMedicamento").val(response.codMedicamento);
			$("#faturamentoMedicamentosEditForm #quantidade").val(response.quantidade);
			$("#faturamentoMedicamentosEditForm #valor").val(response.valor);
			$("#faturamentoMedicamentosEditForm #codLocalAtendimento").val(response.codLocalAtendimento);
			$("#faturamentoMedicamentosEditForm #dataCriacao").val(response.dataCriacao);
			$("#faturamentoMedicamentosEditForm #dataAtualizacao").val(response.dataAtualizacao);
			$("#faturamentoMedicamentosEditForm #codStatus").val(response.codStatus);
			$("#faturamentoMedicamentosEditForm #codAutor").val(response.codAutor);
			...
			$.validator.setDefaults({
				...
				submitHandler: function(form) {
					...
					$(".text-danger").remove();
					$.ajax({
						url: '<?php echo base_url('faturamentoMedicamentos/edit') ?>' ,						
						type: 'post',
						data: form.serialize(), 
						dataType: 'json',
						beforeSend: function() {
							//$('#faturamentoMedicamentosEditForm-btn').html('<i class="fa fa-spinner fa-spin"></i>');
						},								
						success: function(response) {
							...
							$('#faturamentoMedicamentosEditForm-btn').html('Salvar');
						}
					});
...
function removefaturamentoMedicamentos(codFaturamentoMedicamento) {	
	Swal.fire({
	  ...
	  if (result.value) {
		$.ajax({
			url: '<?php echo base_url('faturamentoMedicamentos/remove') ?>',
			type: 'post',
			data: {
				codFaturamentoMedicamento: codFaturamentoMedicamento,
				csrf_sandra: $("#csrf_sandraPrincipal").val(),
