...
class AgendamentosFaltasModel extends Model
{

	protected $table = 'amb_agendamentosfalta';
	protected $primaryKey = 'codAgendamentoFalta';
	...
	public function faltasPaciente($codPaciente)
	{
		$codOrganizacao = session()->codOrganizacao;
		$query = $this->db->query('select af.*, pee.nomeExibicao as nomeEspecialista,e.descricaoEspecialidade,pa.nomeExibicao as nomePaciente, pe.nomeExibicao as nomeAutor
		from amb_agendamentosfalta af
		...
		left join sis_pessoas pee on pee.codPessoa = af.codEspecialista
		left join sis_especialidades e on e.codEspecialidade =af.codEspecialidade 
		where af.codPaciente = '.$codPaciente.' and af.dataInicioImpedimento >= ADDDATE(NOW(), INTERVAL -90 DAY)
		');
		return $query->getResult();
	}


	public function pegaPorCodigo($codAgendamentoFalta)
	{
		$query = $this->db->query('select * from ' . $this->table . ' where codAgendamentoFalta = "' . $codAgendamentoFalta . '"');
		return $query->getRow();
	}
}
