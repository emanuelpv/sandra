...
class TotenModel extends Model
{
	...
	public function filaPrioridades($departamentos = null)
	{
		...
		where codStatus=0 and a.codPrioridade=1 and a.codDepartamento in (' . $departamentos . ') and DATE_FORMAT(a.dataProtocolo, "%Y-%m-%d") >= DATE_FORMAT(NOW(), "%Y-%m-%d") 
		order by a.codPrioridade desc, a.dataProtocolo asc, a.idade desc');
		} else {
			$query = $this->db->query('select *
			from amb_atendimentosenha a
			where codStatus=0 and a.codPrioridade=1 and DATE_FORMAT(a.dataProtocolo, "%Y-%m-%d") >= DATE_FORMAT(NOW(), "%Y-%m-%d") 
			order by a.codPrioridade desc, a.dataProtocolo asc, a.idade desc');
		}
	...
	public function filaNormal($departamentos = null)
	{
		...
		if ($departamentos !== null and $departamentos !== "") {
			$query = $this->db->query('select *
		from amb_atendimentosenha a 
		where codStatus=0 and a.codPrioridade=0 and a.codDepartamento in (' . $departamentos . ')  and DATE_FORMAT(a.dataProtocolo, "%Y-%m-%d") >= DATE_FORMAT(NOW(), "%Y-%m-%d")
		order by a.dataProtocolo asc, a.idade desc');
		} else {
			$query = $this->db->query('select *
			from amb_atendimentosenha a 
			where codStatus=0 and a.codPrioridade=0 and DATE_FORMAT(a.dataProtocolo, "%Y-%m-%d") >= DATE_FORMAT(NOW(), "%Y-%m-%d") 
			order by a.dataProtocolo asc, a.idade desc');
		}
