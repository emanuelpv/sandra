...
class AgendamentosExamesListaModel extends Model
{
	...
	{
		$query = $this->db->query('select codStatus as id,nomeStatus as text from age_agendamentosexamesstatusconfig order by codStatus');
		return $query->getResult();
	}
	...
	}	public function listaDropDownAgendamentosExamesListaDisponivelMarcacaoExame()
	{
		$query = $this->db->query('select distinct e.codExameLista as id,e.descricaoExameLista as text 
		from age_agendamentosexameslista  e
		join age_agendamentosexameslistamembros em on e.codExameLista=em.codExameLista
	...
	{
		$query = $this->db->query('select codTipo as id,nomeTipo as text from age_agendamentosexamestipo order by codTipo');
		return $query->getResult();
	}
	...
	public function pegaAgendamentosExamesLista()
	{
	...
	public function lookupCodNomeAgendamentosExamesListaJson($agendamentosExamesLista)
	{
		$query = $this->db->query('select * from ' . $this->table . ' where codExameLista in (' . $agendamentosExamesLista . ')');
		return $query->getResult();
	}
	public function importarAgendamentosExamesLista($dados)
	{
	...
	public function agendamentosExamesListaApolo()
	{
		$apolo = \Config\Database::connect('apolo', false);
		$query = $apolo->query('select *
		from tp_agendamentosExamesLista');
		return $query->getResult();
	}
	public function membrosAgendamentosExamesListaApolo()
	{
		$apolo = \Config\Database::connect('apolo', false);
		$query = $apolo->query('select *
		from pes_exameLista');
